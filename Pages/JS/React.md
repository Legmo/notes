...
- Основные
  ...
  - [useCallback](https://ru.reactjs.org/docs/hooks-reference.html#usecallback) — мемоизация/кэширование коллбэков. Предотвращать лишний ре-рендер. Полезно когда надо прокидывать колбэки в дочерний компонент.
  - [useMemo](https://ru.reactjs.org/docs/hooks-reference.html#usememo) — мемоизация/кэширование значений.  Предотвращать лишний ре-рендер. Замена PureComponent и shouldComponentUpdate.
...
//Пример с подпиской на функцию из props, удалением подписки при размонтировании компонента и зависимость от props.source
useEffect(() => {
...
- useLayoutEffect — запускается до рендреа. Только когда он закончится - наичанется рендер.
...
- `componentDidUpdate`
  ...
  - Это лучше классического componentDidUpdate — более гибкое управл
...
Возвращает результат функции. Перезапускается только если изменилась зависимость.
