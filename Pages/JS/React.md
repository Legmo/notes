...
- Принимает — значение state или функцию
  - `setCount(newCount)` — принимает значение
  - `setCount(prevCount => prevCount - 1)` — принимает функцию для вычисления значения на основе предыдущего `state`
- Возвращает — новое значение `state`
- Ререндер компонента
  - Если setState() обновил значение state — происходит ререндер компонента.
  - Если setState() вернул тот же результат, что и текущий state — повторного рендеринга не будет.
- Перезапись объекта `state`
  - Если в state хранится объект — `useState` не объединяет объекты, а заменяет старый объект новым. Отличие от
    метода `setState` классовых компонент,
...
Если новое состояние опирается на предыдущее состояние, используйте функцию.<br>
Неправильно:
