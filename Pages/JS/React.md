...
React содержит около 15 [встроенных хуков](https://ru.reactjs.org/docs/hooks-reference.html) (например `useState`).
Можно создавать собственные.

Хуки не работают в классовых компонентах, только в функциональных.

Название хука всегда начинается с `use`. `useState`, `useRef`, `useId`, `useИмяСамодельногоХука`...
