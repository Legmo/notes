...
В одном компоненте можно вызвать несколько useState() — получим несколько отдельных state. Изменение каждого будет
вызывать ререндер компонента.
...
- **Принимает**
  - начальное значение для state. Используется при первом рендеринге компонента.
...
- **Возвращает**
  - значение state
  - функцию для его обновления (метод сеттер). `setState(newState)`
    - принимает значение state или функцию для вычисления значения на основе предыдущего `state`
    - `setCount(initialCount)` — принимает значение
    - `(prevCount => prevCount - 1)` — принимает функцию.
    - Если setState() обновил значение state — происходит ререндер компонента
    - Если setState() возвращает тот же результат, что и текущий state — повторного рендеринга не будет.
    ...
    - Если в state хранится объект — в отличие от метода `setState` классовых компонент, `useState` не объединяет
      объекты, а заменяет старый объект новым.
      - Мы передаём новый объект `state`, который заменит предыдущий => если хотим изменить в объекте только одну строку
        надо делать так:
      - ```js
...
`Асинхронность setState()`<br>
setState() — асинхронная функция. <br>
Под капотом Реакт объединяет все мутации состояний, благодаря чему код функционального компонента будет вызван 1 раз,
это называется "batching".<br>
...
`Досрочное прекращение обновления состояния` — если обновить state хука тем же значением, что и текущее состояние, React
досрочно выйдет из хука без повторного рендера дочерних элементов и запуска эффектов.

`Пакетное обновление состояния` () — React может сгруппировать несколько setState в один повторный рендеринг для
повышения производительности.<br>
До React 18 пакеты обновлялись только внутри обработчиков событий React. <br>
Начиная с React 18, пакетная обработка включена для всех обновлений по умолчанию. <br>
Обратите внимание — обновления из N разных событий, инициированных пользователем, всегда обрабатываются отдельно и не
объединяются (например двойное нажатие кнопки). Это предотвращает логические ошибки.
