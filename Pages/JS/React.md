...
`Hook` (крючок) в программировании — это технология перехвата вызовов функций в чужих процессах. Механизм позволяет
отследить некое событие в системе, «перехватить» его и вызвать в этот момент свою функцию. <br>
Т.е. система сообщает: сейчас я буду делать то-то, есть желающие в этот момент выполнить свою логику? И если какие-то
функции на это событие/хук «подписаны» — они в этот момент будут вызваны. <br>
Т.е. это заложенная в систему возможность:

- уведомлять другие программы/процессы о своих событиях
- вызывать их функции (в момент наступления данных событий).<br>
  Сама концепция хуков используется повсеместно - в операционных системах, CMS и т.д.

Хук в React — функция, с помощью которой можно использовать state и методы жизненного цикла в функц. компонентах. Не
работает в классах.

Позволяет повторно использовать логику состояния, а не сам state.

Каждое обращение к хуку обеспечивает совершенно изолированный state. Можно использовать один и тот же хук
несколько раз в одном компоненте.<br>
...
- либо мы вводим под каждый счётчик отдельное значение в стэйте<br>
  Если я использую хуки - я могу вызывать несколько раз один хук, и он будет по каждый вызов создавать разные локальные
  стэйты, которые независимо хранят данные для каждого счётчика. Как-то так.

React содержит несколько встроенных хуков, таких как useState. Можно создавать собственные хуки, чтобы повторно
