...
Фреймворк
- масштабнее библиотеки, задаёт архитектуру, его сложнее поменять. Как офис для бизнеса - задаёт инфраструктуру (электричество, вода, канализация, парковка, отопление...)
- обычно решает несколько задач
- производит инверсию управления (inversion of control, IOC) —

Библиотека
- не такое масштабное решение, можно легко поменять. Мебель, компьютер, шторы в офисе... Чтоб не создавать свои стулья - купил готовые, но могут легко заменить на другие
...
`Инверсия зависимостей` (Dependency inversion, DI) - буква D в аббревиатуре SOLID. Принцип гласит:
...
Сначала использование node.js-модулей выглядело так: вместо загрузки всего moment.min.js в скриптовом теге HTML можно было грузить
JS-файл напрямую:
```js
...
Загрузка модулей работает прекрасно, т.к. `node.js` — серверный язык с доступом к файловой системе.<br>
...
Инструмент для сборки модулей в единые пакеты, имеющий доступ к файловой системе.<br>
Эти пакеты совместимы с браузером, которому не нужен доступ к файловой системе.<br>
...
Надо только учесть, что когда мы работаем с JSX - мы не видим `React.createElement`.<br> 
