...
- <details><summary><b>Кратко</b></summary><p>
    ...
    - в движке конвейер:
        - `парсер` -> `абст. синтакс. дерево` -> `компилятор` (несколько) -> `машинный код`
        - альтернативное описание:
        - `парсер` -> `абст. синтакс. дерево` -> преобразуется в `оптимизированную форма двоичного байт-кода` -> передаётся `виртуальной машине JS` для выполнения
    - в движке
...
- <details><summary><b>Движок. Окружение (Среда)</b></summary><p>
  ...
  - `Движок` (JS Engine) — программа для обработки JS кода и конвертирования его в конкретные команды для выполнения. Работает в «окружении»
    - Включает в себя
      - Парсер
          - анализирует исходный код 
          - преобразует код в `абстрактное синтаксическое дерево` (AST)
      - Компилятор
          - преобразует AST в `оптимизированную форму двоичного байт-кода`, которуб понимает виртуальная машина
      - Виртуальная машина
      ...
      - API для взаимодействия с окружением
...
- <details><summary><b>Виртуальная машина JS (JS VM)</b></summary><p>
...
- <details><summary><b>Стэк вызовов. Контекст выполнения функции</b></summary><p>
  ...
  - Контекст выполнения функции (execution context) — структура данных. 
    - создается при каждом вызове функции и удаляется со стека, когда функция завершает выполнение. 
