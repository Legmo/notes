...
- родительская функция возвращает дочернюю функцию (через return)
...
- Внутри функции `A()` создадим переменную `Х` и вторую функцию `B()` (назовём её вложенной).
- Вложенная функция `B()` использует переменную `Х`.
- Родительская функция `A()` возвращает вложенную функцию `B()` (через return)
- Создавая разные экземпляры родительской функции `A()` получаем вложенные функции `B()`, каждая из которых помнит своё
  значение `Х`.
...
Это и называется "замыкание" - переменные замкнуты на саму функцию.

Т.е. чтобы создать замыкание:

- в функцию `A` вложить функцию `B`,
- из функции `B` обратиться к переменным функции `A` (родительской)
- вложенную функцию `B` вернуть наружу.
...
- `state для функции` — создаём функцию, у которой есть своё изменяемое состояние.
...
Использование этой техники увеличивает требования к производительности как в части скорости, так и в части потребления
памяти.
