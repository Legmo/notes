...
- <details><summary><b>Кратко</b></summary><p>
  ...
  - `Однопоточность`. JS однопоточный язык. За раз может выполняться только один блок кода. Одна задача в один момент времени. Работа с DOM в 2 потока — плохая идея.
  - `Асинхронность`. Есть две строки кода. Пока первая думает — вторая может уже запуститься (не ожидая завершения работы кода из первой).
  - 
  ...
  - Асинхронные операции в окружении.
  - 
  ...
  - Работа движка
    ...
    - Выполнение кода движком
      ...
      - если встретилась асинхронная задача — отправляется на выполнение в окружение
      - если встретилась синхронная задача — её контекст выполнения помещается в `стэк вызовов` 
    - Стэк вызовов
        - такая стр-ра данных в движке
    ...
    - Асинхронные задачи 
      ...
      - после выполнения (таймер сработал, данные получены) задача ставится в `очередь` как обычная синхронная
      ...
      - `Web workers`
        - Ещё можно использовать `вебворкеры`, но из них нельзя менять DOM или вызывать методы объекта window.



      - `Event loop` (цикл событий) — управляет очередью задач
      - когда стэк вызовов стал пустым (все синхронные задачи выполнены) — event loop проверяет очередь задач.
      - ждёт нужный момент и ставит первую задачу из очереди в стэк вызовов. Дальше всё как обычно: проверяем не асинхронная ли она, если нет — работаем в стэке.
      - event loop работает в бесконечном цикле :
        - `макрозадача 1` -> все `микрозадачи` -> `отрисовка страницы` (рендер) -> `макрозадача 2`
        - каждая итерация = 1 тик
