...
- Логические операторы присваивания `&&=`, `||=`, `??=` — присвоить если...<br>
  - `user &&= 'A'` — если user === true &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; *user && (user = 'A')*
  - `user ||= 'A'` — если user === false [ *if(!user)(user = 'A')* ]
  - `user ??= 'A'` — если user === null или undefined [ *if(user === null || user === undefined)(user = 'A')* ]
...
- Объект `AggregateError` — новый тип ошибок.<br>
  Представить N ошибок в виде одной комбинированной. Например для Promise.any(), когда все promises завершились в
  rejected
