...
В JS объекты имеют специальное скрытое «свойство» `[[Prototype]]` (внутренний слот объекта).<br>
...
- `[[Prototype]]` — внутренний слот объекта (как сскрытое свойство). 
  - Указывает на следующий прототип в цепочке (`null` если это последний прототип).
  - Его нельзя менять напрямую => появились методы для работы с ним. 
