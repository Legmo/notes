...
Ассинхронное поведение нужно когда что-то тормозит.<br>
...
Cвойства объекта `promise` (возвращается конструктором `new Promise`)
- `state` («состояние»)
...
Асинхронные функции на основе promises<br>
Позволяют избавиться от «ада коллбэков» и улучшить внешний вид и читаемость кода.
...
Скрипт скачивается ассинхронно (параллельно с формированием документа). Как только скрипт загружен - он запускается,
...
Скрипт скачивается ассинхронно (параллельно с формированием документа). И после получения - скрипт не запускается сразу,
...
[//]: # (Декоратор Debounce)
<details><summary><b>Декоратор Debounce (Дебаунс)</b></summary><p>
...
<details><summary><b>Декоратор Throttling (Тротлинг)</b></summary><p>
