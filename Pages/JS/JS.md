...
- РЕЗЮМЕ
  ...
  - если объект создан при помощи класса (функции, но не стрелочной) — у него есть свойство `prototype` (X.prototype)
  - `__proto__` объекта ссылается на `prototype` класса (функции-конструктора, `new X()`), которым этот объект был создан 
  - зачем классу нужен `prototype` — чтобы созданные от него объекты могли наследовать методы и свойс
  - `prototype` = свойство, содержит объект со своим набором свойств и методов
    <br>
<br>
