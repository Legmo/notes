...
- Null - преднамеренное отсутствующее значение,
- undefined - неинициализированное значение (не было изначально)
- Ошибки - несколько встроенных типов
...
Примитивы (строки, числа, булевы значения, null/undefined) при присваивании переменных копируются целиком или, как
говорят, **по значению**.
...
В результате получили две независимые переменные, каждая из которых хранит значение "Привет!". Изменение `message` никак
не повлияет на `phrase`

Объекты (в том числе массивы и функции) копируются **по ссылке**.<br>
В переменной, которой присвоен объект, хранится не сам объект, а «адрес его места в памяти», иными словами – «ссылка» на
него.

Если есть две переменные с одной и той же функцией - в них не лежит копия этой функции, а обе эти переменные ссылаются
...
Появился в ES5 (2009)<br>
Добавлен для обратной совместимости - чтоб старые программы корректно работали.<br>
Его надо включать вручную, прописать "use strict" в начале скрипта или функции.<br>
Выше него могут быть написаны только комментарии.
...
- нельзя присваивать значения св-вам, недоступным для записи (read-only).<br> 
...
- Ключевое слово `this` в функции ведет себя по-разному в строгом режиме. Ключевое слово `this` относится к объекту,
  который вызвал функцию. Если объект не указан, функции в строгом режиме вернут, undefined, а функции в нормальном
  режиме вернут глобальный объект (окно):

Современный JavaScript поддерживает «классы» и «модули» — продвинутые структуры языка (и мы, конечно, до них доберёмся),
которые автоматически включают строгий режим. Поэтому в них нет нужды добавлять директиву "use strict".
