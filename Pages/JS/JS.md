...
- Нельзя **присваивать значения read-only св-вам** (недоступным для записи).<br>
  В строгом режиме любое присвоение недоступному для записи свойству, только для получения, несуществующему свойству,
  несуществующей переменной или несуществующему объекту вызовет ошибку.
...
- **Восьмеричные** числовые литералы и escape-символы не допускаются:
- **Зарезервированные слова** `eval`, `arguments`,`with `,``, нельзя использовать в качестве переменной
