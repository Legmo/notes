...
Промисы можно объединять в цепочки — замена «пирамиды вложенных коллбеков».<br>
...
- в коллбек resolve/rejected передаю результат работы или ошибку. Эти данные поступят в обработчик, и там с ними можно
  работать.
- после промиса добавляем к нему конструкции-обработчики (then/catch/finally) - выполнят разные действия, в зависимости
...
- Promise.all
  ...
  - Если любой из промисов завершится с ошибкой, то промис, возвращённый Promise.all, немедленно завершается с этой
    ошибкой. В случае ошибки, остальные результаты игнорируются
...
[//]: # (Промисы - Async/Await)
<details><summary><b>Промисы - Async/Await</b></summary><p>

Специальный синтаксис для работы с промисами.<br>
Замена `.then` — позволяет оформлять асинхронный код более привычным способом, как синхронный.

Асинхронные функции на основе promises<br>
Позволяют избавиться от «ада коллбеков» и улучшить внешний вид и читаемость кода.
