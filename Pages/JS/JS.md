...
- `[[Prototype]]` — скрытое «свойство» объекта. На самом деле внутренний слот. Его нельзя менять напрямую => появились методы для работы с ним. Указывает на следующий прототип в цепочке (`null` если это последний прототип).
- `__proto__` — свойство объекта (устаревшее). Геттер/сеттер для `[[Prototype]]`. 
  - Находится в св-ве `prototype`
  - унаследованное свойство, пришло от `Object.prototype` => когда в цепочке прототипов пропадает ссылка на Object.prototype (например `foo.__proto__ = null; // делаем прототип объекта null`), __proto__ превращается в тыкву и перестает работать с прототипом.
