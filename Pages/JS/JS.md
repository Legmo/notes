...
JS берёт задачу из очереди и выполняет её, когда он освободится от выполнения текущего кода.
...
- создаём специальный объект-промис (при помощи конструктора new Promise()),
...
- по результатам работы функции вызываем коллбеки. Можно вызвать только один, если второй вариант не интересует
- после промиса добавляем к нему конструкции-обработчики (then/catch/finally) - выполняют разные действия, в зависимости
  от результатов выполнения функции (resolve/rejected).
- обработчики .then/.catch тоже возвращает объект-промис - можно строить длинные цепочки
- в конце цепочки обязательно ставить catch/finally

Промисы можно объединять в длинные цепочки - это хорошая замена «пирамиде вложенных коллбеков».<br>
...
Мы сами устанавливаем контекст, в котором выполняется функция. Это означает, что когда мы используем ключевое слово this
внутри нашей callback-функции, оно ссылается на то, что мы передаём первым аргументом в call()/apply
