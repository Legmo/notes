...
Каждая операция содержит `контекст` — некую область памяти, из которой доступны данные.<br>
Контексты расположены в памяти в виде дерева. Каждому листу в дереве доступны области видимости, которые определены в
родительских ветках и в корне (глобальной области видимости).
...
Разница между `JS Engine` (движок) и `JS Run-time Enviroment` (среда).

- `Движок` - это программа, которая используется для обработки заданного кода и конвертирования его в конкретные команды
  для выполнения.
- `Среда` - это среда отвечающая за создание экосистемы с возможностями, сервисами и поддержкой, такими как массивы,
  функции, ключевые библиотеки и тп, которые необходимы для того, чтобы код запустился верно.

Почти все браузеры имеют JS движок. <br>
Самые популярные это:

- V8 в Google Chrome и Node.js,
- SpiderMonkey Мазилы,
- Chakra для IE

- Хоть все эти браузеры и выполняют JS по-разному, но под капотом, они все работают по одной модели: стэк вызова, куча,
  event loop, Web API...
