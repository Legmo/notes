...
[//]: # (Перестановка строки в обратном порядке)
<details id="task-3-2"><summary><b>Перестановка строки в обратном порядке</b></summary><p>

**Задача**
- Есть строка. Нужно вывести все смиволы строки в обратном порядке

**Решение**
- «Заимствование» не работает с методами, изменяющими массивы, т.к. строки неизменны => не могут изменяться «на месте».
- Решение — преобразовать строку в массив, выполнить нужную операцию и преобразовать обратно в строку.
- 
- ```js
    var a = 'Abc';
    var с = а
      .split( "" )// разбиение ·а· на массив символов
      .reverse()// переставить массив символов в обратном порядке
      .join( "" );// снова объединить массив символов в строку
    с; // "cbA"
  ```
- Такое решение не работает для строк, содержащих сложные символы (Unicose), многобайтовые символы и т. д.
- Если вы часто выполняете со «строками» операции, которые интерпретируют их как массивы символов, возможно, проще хранить их в виде массивов вместо строк. Скорее всего, вы избавитесь от постоянных хлопот с преобразованием строк в массивы. А когда вам понадобится строковое представление, для массива символов всегда можно вызвать join ("").
